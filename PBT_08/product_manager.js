// ==========================================================================
// MẢNG DỮ LIỆU ĐẦU VÀO GỐC (PRODUCTS ARRAY)
// ==========================================================================
const products = [
...
// ==========================================================================
// THỰC HIỆN XÂY DỰNG CÁC HÀM XỬ LÝ (SỬ DỤNG HOÀN TOÀN HÀM BẬC CAO)
// ==========================================================================
...
// 2. Lọc theo category VÀ khoảng giá (minPrice <= price <= maxPrice)
...
// 3. Sắp xếp theo giá (tăng/giảm) - Lưu ý: tạo mảng mới bọc spread để không đột biến mảng gốc
...
function cheapestByCategory(products) {
    return products.reduce((acc, currentProduct) => {
        ...
        // Nếu danh mục này chưa được ghi nhận HOẶC sản phẩm hiện tại rẻ hơn sản phẩm đã lưu
...
// 6. Tạo mảng chỉ chứa { name, formattedPrice } theo cấu trúc tiền tệ VNĐ
...
// 7. Tính điểm đánh giá rating trung bình của toàn bộ hệ thống sản phẩm
function averageRating(products) {
    ...
    // Làm tròn lấy 2 chữ số thập phân chuẩn chỉnh
    return Math.round(avg * 100) / 100;
}

// 8. Tìm sản phẩm theo keyword (tìm kiếm mờ không phân biệt chữ hoa chữ thường trong thuộc tính name)
...
// ==========================================================================
// BỘ DỮ LIỆU CHẠY THỬ (TEST CASES THEO ĐỀ BÀI CHỈ ĐỊNH)
// ==========================================================================
...
// Test thêm các hàm bổ trợ còn lại để kiểm tra tính đúng đắn toàn diện
console.log("\n=== SORT BY PRICE DESC ===");
console.log(sortByPrice(products, "desc").map(p => `${p.name}: ${p.price.toLocaleString()}đ`));

console.log("\n=== FORMATTED PRODUCT LIST ===");
console.log(formatProductList(products).slice(0, 3)); // In thử 3 phần tử đầu tiên
